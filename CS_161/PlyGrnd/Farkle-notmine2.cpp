// farkle.cpp : Defines the entry point for the console application.
// Created by Jared Hicks September, 2017
//
//N#include <stdafx.h>
#include <iostream>
#include <string>
...
void readRules() // Function to read text file containing rules text
{

    string fileText;
    ifstream myReadFile;
    myReadFile.open("farkleRules.txt"); //Text file call
    while (getline(myReadFile, fileText)) // Loop to read each line and print it to the window
    {
        std::cout << fileText << '\n';
    }
    myReadFile.close(); //Closes text file once the operation is completed
}
...
int firstTurn(int *playerScore) // First turn that handles the rolls before the initial 500+ roll, executes on it's own until it happens
{
    diceRoll();
    printRoll();
    scoring(playerScore);
...
int main(string* players[])
{
    ...
    std::cout << "Welcome to Farkle, please read the following rules before begining play: " << endl << endl;
    readRules(); //Function to read text file containing rules and display it on screen
    int numPlayers;
    ...
    while (firstScore < 500) { firstTurn(playerScore); } //First turn won't end until player rolls over 1000 on a single roll
